...
try:
  regularity_intra = float(sys.argv[sys.argv.index('--regularity_intra')+1])
except:
  regularity_intra = 50000.0
...
try:
  syns = sys.argv[sys.argv.index('--synapses')+1].split(',')
except:
  syns = []
...
for name, n in sorted(cfg.cfg.numCells.items()):
  ...
  for i in range(n):
    ...
    if name in syns:
      ...
      burst_model.rate = np.zeros(burst_model.time.size) + cfg.cfg.ratesShort[fr_name] * factor
      ...
      burst_model.inter_rate = np.zeros(burst_model.inter_time.size) + 0.5
      burst_model.min_inter_period = Tdur

      burst_model.intra_distribution = mcn.Distribution((seed+3000+1, 1), 'gamma')
      burst_model.intra_distribution.k = regularity_intra
      burst_model.intra_distribution.theta = 1/regularity_intra
